...
GhostManager::GhostManager(int level) : level(level) {
	...
	ghosts[1] = Ghost(ghosts[0].pos.x, ghosts[0].pos.y + C_SIZE * 3, 0, PINK);
	ghosts[2] = Ghost(ghosts[1].pos.x + C_SIZE * 2, ghosts[1].pos.y, FPS * (10 - level), ORANGE);
	ghosts[3] = Ghost(ghosts[1].pos.x - C_SIZE * 2, ghosts[1].pos.y, FPS * (6 - level), SKYBLUE);
	...
	keypoint1 = 160 + level * 10;
	keypoint2 = 80 + level * 10;
...
void GhostManager::update(vec2 playerPos, vec2 playerDir, int dotsRemaining) {
	...
	if (dotsRemaining < keypoint1 && cruiseElroyMode == 0) {
		cruiseElroyMode = 1;
		ghosts[0].speed *= 1.05;
	}
